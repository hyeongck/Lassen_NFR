...
namespace LibEqmtDriver.SCU
{
    public class AeWofer : iSwitch  
    {
        ...
        {
            //Note : Updated AeWolfer (to inc SP8T switch) has 2x IO when compared with original only 1x IO
            //example Val data format(Dec Value) : xxxx;yyyy where xxxx = 1st IO and yyyy = 2nd IO
            string[] tempdata;
            tempdata = val.Split(';');
            ...
            try
            {
                ...
                {
                    AMB1340C_DRIVEPIN(i, Convert.ToInt32(tempdata[i]));
                    
                }
            }
            catch (Exception ex)
            {
                throw new Exception("AeWolfer: SetPath -> " + ex.Message);
            }
        }
