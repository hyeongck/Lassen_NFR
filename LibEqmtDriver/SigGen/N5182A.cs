...
namespace LibEqmtDriver.SG
{
    public class N5182A : iSiggen
    {
        ...
        public void OpenIO()
        {
            if (IOAddress.Length > 3)
            {
                try
                {
                    ...
                    myVisaSg.IO = (IMessage)mgr.Open(IOAddress, AccessMode.NO_LOCK, 2000, "");
                }
        ...
        void iSiggen.EnableRF(INSTR_OUTPUT _ONOFF)
        {
            myVisaSg.WriteString("OUTP:STATE " + _ONOFF, true);

        }
        void iSiggen.EnableModulation(INSTR_OUTPUT _ONOFF)
        {
            myVisaSg.WriteString("OUTP:MOD " + _ONOFF, true);
        }
        void iSiggen.SetAmplitude(float _dBm)
        {
            myVisaSg.WriteString(":POW " + _dBm.ToString(), true);
        }
        void iSiggen.SetFreq(double _MHz)
        {
            myVisaSg.WriteString("FREQ " + _MHz.ToString() + "MHz", true);
        }
        void iSiggen.SetPowerMode(N5182_POWER_MODE _mode)
        {
            myVisaSg.WriteString(":SOUR:POW:MODE " + _mode.ToString(), true);
        }
        void iSiggen.SetFreqMode(N5182_FREQUENCY_MODE _mode)
        {
            myVisaSg.WriteString(":SOUR:FREQ:MODE " + _mode.ToString(), true);
        }
        void iSiggen.MOD_FORMAT_WITH_LOADING_CHECK(string strWaveform, string strWaveformName, bool WaveformInitalLoad)
        {
            while (true)
            {
        ...
        void iSiggen.SELECT_WAVEFORM(N5182A_WAVEFORM_MODE _MODE)
        {
            switch (_MODE)
            {
        ...
        void iSiggen.SET_LIST_TYPE(N5182_LIST_TYPE _mode)
        {
            myVisaSg.WriteString(":LIST:TYPE " + _mode.ToString(), true);
        }
        void iSiggen.SET_LIST_MODE(INSTR_MODE _mode)
        {
            myVisaSg.WriteString(":LIST:MODE " + _mode.ToString(), true);
        }
        void iSiggen.SET_LIST_TRIG_SOURCE(N5182_TRIG_TYPE _mode)
        {
            myVisaSg.WriteString(":LIST:TRIG:SOUR " + _mode.ToString(), true);
        }
        void iSiggen.SET_CONT_SWEEP(INSTR_OUTPUT _ONOFF)        // Set up for single sweep
        {
            myVisaSg.WriteString(":INIT:CONT " + _ONOFF.ToString(), true);
        }
        void iSiggen.SET_START_FREQUENCY(double _MHz)
        {
            myVisaSg.WriteString("FREQ:START " + _MHz.ToString() + "MHz", true);
        }
        void iSiggen.SET_STOP_FREQUENCY(float _MHz)
        {
            myVisaSg.WriteString("FREQ:STOP " + _MHz.ToString() + "MHz", true);
        }
        void iSiggen.SET_TRIG_TIMERPERIOD(double _ms)
        {
            myVisaSg.WriteString("TRIG:SEQ:TIM " + _ms.ToString() + "ms", true);
        }
        void iSiggen.SET_SWEEP_POINT(int _points)
        {
            myVisaSg.WriteString("SWE:POIN " + _points.ToString(), true);
        }
        void iSiggen.SINGLE_SWEEP()
        {
            myVisaSg.WriteString("SOUR:TSWEEP", true);
        }
        void iSiggen.SET_SWEEP_PARAM(int _points, double _ms, double _startFreqMHz, double _stopFreqMHz)
        {
        ...
        bool iSiggen.OPERATION_COMPLETE()
        {
            try
            {
        ...
        void iSiggen.SET_ROUTE_CONN_EVENT(N5182A_ROUTE_SUBSYS _MODE)
        {
            switch (_MODE)
            {
        ...
        void iSiggen.SET_ROUTE_CONN_TOUT(N5182A_ROUTE_SUBSYS _MODE)
        {
            switch (_MODE)
            {
        ...
        void iSiggen.SET_ROUTE_CONN_SOUT(N5182A_ROUTE_SUBSYS _MODE)
        {
            switch (_MODE)
            {
                case N5182A_ROUTE_SUBSYS.SweepOut:
        ...
        void iSiggen.SET_ALC_TRAN_REF(N5182A_ALC_TRAN_REF _MODE)
        {
            switch (_MODE)
            {
        ...
        void iSiggen.QueryError_SG(out bool status)
        {
            status = false;
